...
import { EXAM_CONFIG } from "@/lib/kpsea-kenya/config";

describe("KPSEA Kenya Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM124");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("kpsea-kenya");
  });
